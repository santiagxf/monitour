...
- **Per-monitor 2-D histogram + permanent geometric prior in `PassiveCalibrator`.** Constant-time update (Gaussian splat into ~2400 cells), constant memory, no training loop. The histogram replaces an earlier single-Gaussian-per-monitor model that failed on asymmetric layouts (e.g. wide main + small laptop) because the wide screen's σ grew to eat the laptop. The geometric prior — center + angular half-extent computed from each screen's pixel rect at an assumed 60 cm viewing distance — folds Windows' authoritative layout into classification as a permanent Bayesian term whose weight decays as evidence accumulates (anchor / (anchor + n)) but never disappears, so wide-vs-narrow asymmetry stays informative forever. A neural classifier would need labeled data we don't have and per-user fine-tuning we won't ship.
